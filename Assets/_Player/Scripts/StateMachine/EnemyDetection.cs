...
public class EnemyDetection : MonoBehaviour
{
    ...
    public void QueryEnemyInRange(float range , out Transform nearest_obj)
    {
        nearest_obj = null;
        colliders = Physics.OverlapSphere(transform.parent.position ,range ,layer);

        foreach(Collider collider in colliders)
        {
            if(collider.TryGetComponent(out SimpleEnemy _obj))
            {
                nearest_obj = _obj.transform; // Assign the nearest enemy if found
